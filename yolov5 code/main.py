...
if __name__ == "__main__":
    ...
    voc.add_text(filename='143.jpg', path='/home/w/zxl/-40/143.jpg', database='Unknown', segmented=0)
    voc.add_size(1024, 712, 3)

    with open(path + 'B00001.txt', encoding='utf-8') as file:
        for row in file.readlines():
            # content = file.readline()
            s = row.split()
            # //输入xmax和ymax的值
            voc.add_object(name='depth:'+ s[2], pose='Unspecified', truncated=0, difficult=0,\
                           xmax=float(s[0])-0.001, xmin=float(s[0])+0.001, ymax=float(s[1])-0.001, ymin=float(s[1])+0.001)
    voc.build('./test' + str(line) + '.xml')
