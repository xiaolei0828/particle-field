        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                if len(label.split('\n')) == 1:
                    ...
                    p2_tem = p2[0], (p2[1] - (2 * h))
                    cv2.rectangle(self.im, p1, p2_tem, color, -1, cv2.LINE_AA)  # filled
                    x, y0 = p1[0], (p1[1] - 2 if outside else p1[1] + h + 2) - h - 15
                    dy = h + 5
                    for i, txt in enumerate(label.split('\n')):
                        y = y0 + i * dy
                        cv2.putText(self.im, txt, (x, y),  0, self.lw / 3,txt_color,
                                    thickness=tf, lineType=cv2.LINE_AA)
